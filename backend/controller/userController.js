...
export const deleteCourse = async(req, res) => {
    try{
        ...
        return res.status(200).json({ message : `Course ${getCourse.name} removed successfully`});
...
export const getAssignedCourses = async(req, res) =>{
    ...
    try{
        ...
        if(!user.courses || user.courses.length === 0){
            return res.status(200).json({ message : 'No Courses assigned', course : []});
        }
        return res.status(200).json({ count : user.courses.length, course : user.courses});
