...
export const getAll = async(req, res) => {
   try{
    ...
    res.status(500).json({message : 'Error while fetching all courses', error : err.message});
   }
};
...
//create course
 export const createCourse = async(req, res) => {
    try{
        ...
        res.status(500).json({ message : 'Server error while creating new course', error: err.message })
    }
};
...
export const updateCourse = async (req, res) =>{
    ...
    try{
        ...
        res.status(200).json({message : `Course ${courseId} updated successfully`,updatedCourse});

    }catch(err){
        res.status(500).json({message : 'Error while updating the course',error : err.message});
    }
...
export const deleteCoursefromList = async (req, res) =>{
    try{
        ...
        res.status(200).json({ message : `Course ${courseId} has been removed successfully`});

    }catch(err){
        res.status(500).json({ message : 'Error while deleting the course', error : err.message});
    }
};
